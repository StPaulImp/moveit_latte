...
{
  // ROS_WARN("runtime_state_:%d",runtime_state_);
  // ROS_WARN("position_controller_running_:%d",position_controller_running_);
  // ROS_WARN("velocity_controller_running_:%d",velocity_controller_running_);
  // ROS_WARN("joint_forward_controller_running_:%d",joint_forward_controller_running_);
  // ROS_WARN("cartesian_forward_controller_running_:%d",cartesian_forward_controller_running_);
  // ROS_WARN("twist_controller_running_:%d",twist_controller_running_);
  ...
  for (auto& controller_it : stop_list)
  {
    for (auto& resource_it : controller_it.claimed_resources)
    {
      if (checkControllerClaims(resource_it.resources))
      {
        ...
        {
          ROS_ERROR_STREAM("stop_list scaled_controllers::ScaledPositionJointInterface resource_it.hardware_interface = " << resource_it.hardware_interface);
          position_controller_running_ = false;
        }
        if (resource_it.hardware_interface == "hardware_interface::PositionJointInterface")
        {
          ROS_ERROR_STREAM("stop_list hardware_interface::PositionJointInterface resource_it.hardware_interface = " << resource_it.hardware_interface);
          position_controller_running_ = false;
        }
  ...
  for (auto& controller_it : start_list)
  {
    for (auto& resource_it : controller_it.claimed_resources)
    {
      if (checkControllerClaims(resource_it.resources))
      {
        if (resource_it.hardware_interface == "scaled_controllers::ScaledPositionJointInterface")
        {
          ROS_ERROR_STREAM("start_list scaled_controllers::ScaledPositionJointInterface resource_it.hardware_interface = " << resource_it.hardware_interface);
          position_controller_running_ = true;
        }
        if (resource_it.hardware_interface == "hardware_interface::PositionJointInterface")
        {
          ROS_ERROR_STREAM("start_list hardware_interface::PositionJointInterface resource_it.hardware_interface = " << resource_it.hardware_interface);
          position_controller_running_ = true;
        }
        if (resource_it.hardware_interface == "scaled_controllers::ScaledVelocityJointInterface")
        {
